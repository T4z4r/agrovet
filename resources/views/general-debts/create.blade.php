                ...
                <form method="POST" action="{{ route('web.general-debts.store') }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" id="debtor_name" name="debtor_name" value="{{ old('debtor_name') }}" required>
                            @error('debtor_name')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="debtor_phone" class="form-label">Phone</label>
                            <input type="text" class="form-control" id="debtor_phone" name="debtor_phone" value="{{ old('debtor_phone') }}">
                            @error('debtor_phone')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label for="debtor_email" class="form-label">Email</label>
                            <input type="email" class="form-control" id="debtor_email" name="debtor_email" value="{{ old('debtor_email') }}">
                            @error('debtor_email')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="amount" class="form-label">Amount</label>
                            <input type="number" class="form-control" id="amount" name="amount" value="{{ old('amount') }}" min="0.01" step="0.01" required>
