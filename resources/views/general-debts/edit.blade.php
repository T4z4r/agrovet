...
@section('title', 'Edit General Debt')
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Edit General Debt</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('web.general-debts.update', $debt) }}">
                    @csrf
                    @method('PUT')
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-control" id="shop_id" name="shop_id" required>
                                ...
                                @foreach($shops as $shop)
                                    <option value="{{ $shop->id }}" {{ old('shop_id', $debt->shop_id) == $shop->id ? 'selected' : '' }}>{{ $shop->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="date" class="form-control" id="debt_date" name="debt_date" value="{{ old('debt_date', optional($debt->debt_date)->format('Y-m-d')) }}" required>
                            @error('debt_date')<div class="text-danger small">{{ $message }}</div>@enderror
                        </div>
                        <div class="col-md-4 mb-3">
                            <label for="due_date" class="form-label">Due Date</label>
                            <input type="date" class="form-control" id="due_date" name="due_date" value="{{ old('due_date', optional($debt->due_date)->format('Y-m-d')) }}">
